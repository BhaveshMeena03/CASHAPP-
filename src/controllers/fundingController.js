...
async function addPaymentMethod(req, res, next) {
    try {
        ...
        if (!user.stripe_customer_id) throw ApiError.badRequest('User has no Stripe customer — re-register or contact support');
...
async function removePaymentMethod(req, res, next) {
    try {
        ...
        if (pm.user_id !== req.user.id) throw ApiError.forbidden('Not your payment method');
...
async function cashIn(req, res, next) {
    try {
        ...
        if (!amount || amount < 100 || amount > 10000000) {
            throw ApiError.badRequest('Amount must be between $1.00 (100 cents) and $100,000 (10000000 cents)');
        }
        ...
        const user = await userModel.findById(req.user.id);
        if (!user || !user.stripe_customer_id) throw ApiError.badRequest('No Stripe customer found');
        ...
        if (paymentIntent.status === 'succeeded') {
            ...
            return res.status(201).json({
                success: true,
                data: { movement: await fundingModel.findFundMovementByStripeRef(paymentIntent.id) },
        ...
        throw ApiError.badRequest(`Payment failed with status: ${paymentIntent.status}`);
    } catch (err) {
        next(err);
    }
}
...
async function cashOut(req, res, next) {
    ...
    try {
        ...
        res.status(201).json({
            success: true,
            data: { movement: { ...movement, stripe_reference_id: payout.id, status: 'completed' } },
...
async function getFundingHistory(req, res, next) {
    try {
        ...
        const result = await fundingModel.findFundMovementsByUserId(req.user.id, { page, limit });
