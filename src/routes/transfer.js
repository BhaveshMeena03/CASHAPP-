...
const sendSchema = {
    cashtag: { type: 'string', required: true, message: 'Receiver cashtag is required' },
    amount: { type: 'number', required: true, min: 1, message: 'Amount must be a positive number (in cents)' },
    note: { type: 'string', required: false, max: 500 },
};

const requestSchema = {
    cashtag: { type: 'string', required: true, message: 'Cashtag to request from is required' },
    amount: { type: 'number', required: true, min: 1, message: 'Amount must be a positive number (in cents)' },
    note: { type: 'string', required: false, max: 500 },
};
...
router.post('/respond/:transactionId', auth, validate(respondSchema), controller.respondToRequest);
